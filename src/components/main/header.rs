...
use crate::components::main::manage::manage_menu_items;
    ...
    view! {
        ...
        <LayoutHeader class="demo-header" style>
            <Space on:click=move |_| {
                ...
                <div class="demo-name">"Stalwart"</div>
            </Space>
            <Space>
                <AutoComplete
                    value=search_value
                ...
                >
                    ...
                    <div style="height: 70vh; overflow: auto;">
                        ...
                        // {vec![manage_menu_items().into_view()]}
                ...
                <Space class="demo-header__right-btn">
                    ...
                    <Button
                        ...
                        on_click=move |_| {
                            let navigate = use_navigate();
                            navigate("/manage/button", Default::default());
                        }
                    >
...
fn gen_search_all_options() -> Vec<AutoCompleteOption> {
    let mut options: Vec<_> = manage_menu_items()
        .into_iter()
        .flat_map(|group| {
            group.children.into_iter().map(|item| AutoCompleteOption {
                value: format!("/manage/{}", item.value),
                label: item.label,
            })
        })
        .collect();
...
fn use_menu_value(change_theme: Callback<()>) -> RwSignal<String> {
    ...
    let menu_value = create_rw_signal({
        let mut pathname = location.pathname.get_untracked();
        if pathname.starts_with("/manage/") {
